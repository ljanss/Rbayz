...
// ----------------- kernelMatrix class --------------------
   ...
   kernelMatrix::kernelMatrix(varianceSpec var_descr) : kernelMatrix(var_descr, 90.0) {  // set default dim_pct = 90
   }
...
//    but labeledMatrix' version that also handles copying row and column labels.
   ...
   void kernelMatrix::addKernel(kernelMatrix* K2, double rrankpct) {
