//
//  simpleMatrix (storing doubles) and simpleIntMatrix (storing ints) classes.
//
...
//  --------- simpleIntMatrix class, same as simpleMatrix but storing integers

simpleIntMatrix::simpleIntMatrix(size_t nr, size_t nc) {
   doalloc(nr, nc);
   std::fill_n(data0, nr*nc, 0);
}

simpleIntMatrix::simpleIntMatrix(Rcpp::RObject X) {
   if(Rf_isMatrix(X)) {
...
// useCol sets the number of columns to use/copy.
// There is also an initWith without useCol to copy the whole Rcpp matrix.
void simpleIntMatrix::initWith(Rcpp::IntegerMatrix M, size_t useCol) {
   if (nrow> 0 || ncol>0 ) {    // oops this matrix is already allocated
      throw(generalRbayzError("Attempted re-init or re-alloc in simpleMatrix"));
   }
   if (useCol > unsigned(M.ncol()) || useCol <=0) {
   ...
   doalloc(M.nrow(), useCol);
   int *col;  // temporary column pointer
   for(size_t icol=0; icol<useCol; icol++) {
      col=data[icol];
      for(size_t irow=0; irow< unsigned(M.nrow()); irow++) {
         col[irow] = M(irow,icol);
...
void simpleIntMatrix::initWith(Rcpp::IntegerMatrix M) {
   initWith(M, M.ncol());
}
...
// memory alloc. This may fail, but it will be caught in try-catch in main function
void simpleIntMatrix::doalloc(size_t nr, size_t nc) {
   if (nr <= 0 || nc <= 0) {
      throw(generalRbayzError("Zero or negative sizes in initialisation in simpleMatrix"));
   }
   data0 = new int[nr*nc];    // a block of memory nrows * ncolumns
   data  = new int*[nc];      // a list of column-pointers to index it by column
   for(size_t i=0; i<nc; i++)
      data[i] = data0 + i*nr;
   nrow = nr; ncol = nc;
}
